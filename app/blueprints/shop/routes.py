...
def seed_stripe_products():
    ...
    def seed_data():
        ...
        for p in stripe.Product.list().get('data'):
            list_to_store_in_db.append(StripeProduct(stripe_product_id=p['id'], name=p['name'], image=p['images'][0], description=p['description'], price=int(float(p['metadata']['price'])), tax=int(float(p['metadata']['tax']))))
